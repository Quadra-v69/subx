...
function readApiKeys() {
    const configPath = path.join(process.env.HOME, '.config', 'subx', 'config.ini');
...
async function getDnsServers() {
    const resolveFilePath = path.join(os.homedir(), '.config', 'subx', 'resolvers.txt');
...
async function main() {
    try {
        ...
        const configDirectory = path.join(homeDirectory, '.config', 'subx');
        ...
        if (!fs.existsSync(configDirectory)) {
            ...
            // Download default_wordlists and save them in ~/.config/subx folder
            const wordlistUrls = [
        ...
        console.log(clc.greenBright(figlet.textSync('subx', 'Poison')));
